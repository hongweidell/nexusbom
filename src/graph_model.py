...
class NexusBOM:
    ...
    def add_specification(self, spec_id, name, requirement, category,
                          **attributes):
        self.graph.add_node(
            spec_id,
            node_type=NodeType.SPECIFICATION.value,
            name=name,
            requirement=requirement,
            category=category,
            dell_requirement=requirement,
            supplier_feedback=None,
            dell_response=None,
            created_at=datetime.now().isoformat(),
            version=1,
            **attributes
        )
        self._log_change("ADD_SPEC", spec_id,
                         {"requirement": requirement})
        return spec_id
    ...
    def add_quote(self, quote_id, supplier_id, spec_id,
                  response, unit_cost=None,
                  meets_requirement=None, **attributes):
        self.graph.add_node(
            quote_id,
            node_type=NodeType.QUOTE.value,
            response=response,
            unit_cost=unit_cost,
        ...
        self.graph.add_edge(spec_id, quote_id,
                            edge_type=EdgeType.QUOTED_BY.value)
        self.graph.add_edge(quote_id, supplier_id,
                            edge_type=EdgeType.SUBMITTED_BY.value)
        self._log_change("ADD_QUOTE", quote_id, {
    ...
    def add_cost_entry(self, cost_id, component_id,
                       cost_type, amount, currency="USD",
                       **attributes):
        self.graph.add_node(
            cost_id,
            node_type=NodeType.COST_ENTRY.value,
            cost_type=cost_type,
            amount=amount,
            currency=currency,
            created_at=datetime.now().isoformat(),
        ...
        self.graph.add_edge(component_id, cost_id,
                            edge_type=EdgeType.HAS_COST.value)
        return cost_id

    def link_component_to_spec(self, component_id, spec_id):
        self.graph.add_edge(component_id, spec_id,
                            edge_type=EdgeType.REQUIRES_SPEC.value)

    def link_component_dependency(self, component_id, depends_on_id):
        self.graph.add_edge(component_id, depends_on_id,
                            edge_type=EdgeType.DEPENDS_ON.value)

    def link_spec_affects_spec(self, spec_id, affected_spec_id):
        self.graph.add_edge(spec_id, affected_spec_id,
                            edge_type=EdgeType.AFFECTS.value)

    def analyze_change_impact(self, node_id):
        impact = {
            "changed_node": node_id,
            "changed_node_data": dict(self.graph.nodes[node_id]),
            "affected_components": [],
            "affected_specs": [],
            "affected_quotes": [],
            "affected_costs": [],
            ...
            if current != node_id:
                if node_type == NodeType.COMPONENT.value:
                    impact["affected_components"].append({
                        "id": current, **node_data
                    })
                elif node_type == NodeType.SPECIFICATION.value:
                    impact["affected_specs"].append({
                        "id": current, **node_data
                    })
                elif node_type == NodeType.QUOTE.value:
                    impact["affected_quotes"].append({
                        "id": current, **node_data
                    })
                elif node_type == NodeType.COST_ENTRY.value:
                    impact["affected_costs"].append({
                        "id": current, **node_data
                    })
                    impact["total_cost_impact"] += node_data.get(
                        "amount", 0
                    )
                elif node_type == NodeType.SUPPLIER.value:
                    impact["affected_suppliers"].append({
                        "id": current, **node_data
                    })
    ...
    def calculate_total_bom_cost(self):
        ...
        for node_id, data in self.graph.nodes(data=True):
            if data.get("node_type") == NodeType.COST_ENTRY.value:
                ...
                for pred in predecessors:
                    ...
                    if pred_data.get("node_type") == \
                       NodeType.COMPONENT.value:
                        comp_name = pred_data.get("name", pred)
                        breakdown_by_component.setdefault(
                            comp_name, 0
                        )
        ...
        return {
            "total_bom_cost": round(total, 2),
            "by_component": breakdown_by_component,
            "by_cost_type": breakdown_by_type
        }
        ...
        for successor in self.graph.successors(spec_id):
            ...
            if edge_data.get("edge_type") == EdgeType.QUOTED_BY.value:
                ...
                for quote_successor in self.graph.successors(successor):
                    sup_edge = self.graph.edges[
                        successor, quote_successor
                    ]
                    if sup_edge.get("edge_type") == \
                       EdgeType.SUBMITTED_BY.value:
                        quote_data["supplier_name"] = \
                            self.graph.nodes[quote_successor].get("name")
                        quote_data["supplier_id"] = quote_successor
                quotes.append(quote_data)

        quotes.sort(
            key=lambda q: q.get("unit_cost") or float
